...
const ProductList: React.FC = () => {
  ...
  const handleModify = (item: Product) => {
    router.push(`/dashboard/products/edit/${item._id}`);
  };
  ...
  const deleteProduct = useMutation({
    mutationFn: (payload: { id: string }) => DELETE(`/products/${payload.id}`),
    onSuccess: () => {
  ...
  const handleDeleteItem = async () => {
    deleteProduct.mutate({ id: itemId });
  };

  return (
    <div className="bg-white h-full pl-5 pr-16 pt-12 flex flex-col text-black">
      <div className="flex flex-row justify-between items-center">
        <p className="mb-3 font-semibold text-2xl">Gestion des Produits</p>
        <button
          onClick={() => {
