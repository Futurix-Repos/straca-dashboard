...
const Sidebar: React.FC<Props> = ({ items, onShowPopup }) => {
  ...
  return (
    <>
      ...
      >
        <i className={`fas fa-${isOpen ? "times" : "bars"} text-black`} />
      </button>
      <aside
        className={`h-screen absolute md:sticky z-10 top-0 bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out transform ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        } md:translate-x-0 justify-between `}
      >
        <div
          className="w-72 flex flex-col items-center justify-center p-4 hover:cursor-pointer"
          onClick={handleClick}
        >
        ...
        <nav className="flex-1 h-[90%] ">
          <div className="flex flex-col h-full justify-between ">
            <ul className="py-4 mr-10">
              {items.map((item, index) => (
                <Fragment key={index}>
                  <li
                    className={clsx(
                      isActive(item.url) ? "bg-[#3D75B0] text-white" : "",
                      item === item ? "font-semibold" : "",
                      `mb-6 ml-8 p-2.5 rounded-md text-gray-500 hover:text-white hover:bg-[#3D75B0] transition duration-300 hover:cursor-pointer`,
                    )}
                    onClick={() => {
                      handleClickSidebarItem(item);
                    }}
                  >
                    <Link
                      href={item.url}
