...
export const UpdateUserPermissionModal: React.FC<
  UpdateUserPermissionModalProps
> = ({ visible, onClose, selectedUser }) => {
  const customStyles = {
  ...
  const [userPermissions, setUserPermissionsData] = useState<Permission[]>(
  ...
  const formatPermissionsData = useMemo(() => {
    return permissionsData.map((permission) => {
      return {
        ...
        label: `${checkPermissionActionToDisplay(permission)} - ${checkPermissionNameToDisplay(permission)}`,
      };
    });
  }, [permissionsData]);
  ...
  const onSave = useCallback(async () => {
    ...
    if (
      ...
      userPermissions.length === selectedUser?.permissions.length &&
      userPermissions.every((up) => {
        ...
        <div className="p-5 flex flex-col rounded-2xl bg-white text-black">
          ...
          <div className="flex flex-col space-y-4">
            ...
            <Select
              ...
              value={formatUserPermissionsData}
              options={formatPermissionsData}
            />
            {error.length > 0 && (
